...
# 1. Check packages without carrying over names
success <- suppressWarnings(sapply(packs, require, character.only = TRUE, USE.NAMES = FALSE))

# 2. Extract failed packages using indexing instead of names
failed_packs <- packs[!success]

# 3. Install missing packages
if (length(failed_packs) > 0) {
  install.packages(failed_packs)
  
  # 4. Retry loading missing packages without names
  sapply(failed_packs, require, character.only = TRUE, USE.NAMES = FALSE)
}
...
source_lines(file.path(project_root, "02_rscripts/01_data_cleaning.R"), (20:22))
```
...
source_lines(file.path(project_root, "02_rscripts/01_data_cleaning.R"), (28:174))

```


```{r}
#| label: data-cleaning-verbose
#| eval: false
#| message: false
#| warning: false
#| code-fold: true
...
source_lines(file.path(project_root, "02_rscripts/02_descriptives.R"), (31:188))
