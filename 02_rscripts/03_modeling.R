...
# Note: Columns 2–6 report estimates from regression models that include 
# controls for age and gender,
# as well as fixed effects for education levels and income brackets.
# Column 3 includes respondents that did not report their car’s fuel and/or 
# emission category.
# Columns 4–6 include dummies for past Lega vote in legislative, regional, 
...
dataskim <- modelsummary::datasummary_skim(
  dat ,
  fun_numeric = list(
    Unique = NUnique,
...
diesel_tbl <- tinytable::tt(tabdiesel, width = 1, digits = 0)
...
Legaswitch_legis_lm_ms <- estimatr::lm_robust(
  sw_to_lega_18_19 ~ dummy_diesel + dummy_euro_4 + diesel_euro4,
  data = data,
  subset = c(target != 3 &
               target != 4 & no_answer_euro == 0 & no_answer_2018 == 0)
)
...
Legaswitch_legis_lm_cont <- estimatr::lm_robust(
  sw_to_lega_18_19 ~ dummy_diesel + dummy_euro_4 + diesel_euro4 +
    age + female + EDU + INC,
  data = data,
  subset = c(target != 3 & target != 4 &
               no_answer_euro ==
               0  & no_answer_2018 == 0)
)
# 3. Including unkowncar and assigning the treatment
Legaswitch_legis_lm_cont_Unknowncar <- estimatr::lm_robust(
  sw_to_lega_18_19 ~ dummy_diesel_ass + dummy_euro_4_ass + diesel_euro4_ass +
    age + female + EDU + INC +
    dummy_car_unknown,
  data = data,
  subset = c(target != 3 & no_answer_euro == 0 & no_answer_2018 == 0)
)
...
Legaswitch_legis_all <- bind_rows(
  M.Legaswitch_legis_lm_ms,
  M.Legaswitch_legis_lm_cont,
  M.Legaswitch_legis_lm_cont_Unknowncar,
  .id = "Model"
)

Legaswitch_legis_all <- subset(
  Legaswitch_legis_all,
  term == "dummy_diesel" | term == "dummy_euro_4" |
    term == "diesel_euro4" |
    term == "dummy_diesel_ass" |
    term == "dummy_euro_4_ass" | term == "diesel_euro4_ass"
)
Legaswitch_legis_all <- Legaswitch_legis_all |>
  mutate(
    term2 = case_when(
      term == "dummy_diesel" | term == "dummy_diesel_ass" ~ "Diesel",
      term == "dummy_euro_4" |
        term == "dummy_euro_4_ass" ~ "Euro 4",
      term == "diesel_euro4" |
        term == "diesel_euro4_ass" ~ "Diesel Euro 4"
    )
  )

Legaswitch_legis_all$term2 <- factor(Legaswitch_legis_all$term2)

Legaswitch_legis_all$Model <- factor(
  Legaswitch_legis_all$Model,
  levels = c("1", "2", "3"),
  labels = c("No Controls", "With Controls", "Incl. Noncategorized Cars")
)
...
coefplot_2018Legislative <-
  ggplot(Legaswitch_legis_all,
         aes(term3, estimate, colour = term3, fill = term3)) +
...
# 1. no control
Legaswitch_region_lm_ms <- estimatr::lm_robust(
  sw_to_lega_reg_19 ~ dummy_diesel + dummy_euro_4 + diesel_euro4,
  data = data,
  subset = c(
    target != 3 &
      target != 4 & no_answer_euro == 0 & no_answer_regional == 0
  )
)
# 2. with controls
Legaswitch_region_lm_cont <- estimatr::lm_robust(
  sw_to_lega_reg_19 ~ dummy_diesel + dummy_euro_4 + diesel_euro4 +
    age + female + EDU + INC,
  data = data,
  subset = c(
    target != 3 & target != 4 &
      no_answer_euro ==
      0 & no_answer_regional == 0
  )
)
# 3. Assigning Unknown-car models based on self-report of ban effect
Legaswitch_region_lm_cont_Unknowncar <- estimatr::lm_robust(
  sw_to_lega_reg_19 ~ dummy_diesel_ass + dummy_euro_4_ass + diesel_euro4_ass +
    age + female + EDU + INC +
    dummy_car_unknown,
  data = data,
  subset = c(target != 3 &
...
Legaswitch_region_all <- bind_rows(
  M.Legaswitch_region_lm_ms,
  M.Legaswitch_region_lm_cont,
  M.Legaswitch_region_lm_cont_Unknowncar,
  .id = "Model"
)
Legaswitch_region_all <- subset(
  Legaswitch_region_all,
  term == "dummy_diesel" | term == "dummy_euro_4" |
    term == "diesel_euro4" |
    term == "dummy_diesel_ass" |
    term == "dummy_euro_4_ass" | term == "diesel_euro4_ass"
...
Legaswitch_region_all$Model <- factor(
  Legaswitch_region_all$Model,
  levels = c("1", "2", "3"),
  labels = c("No Controls", "With Controls", "Incl. Noncategorized Cars")
)
...
# 1. no control
Legaswitch_munic_lm_ms <- estimatr::lm_robust(
  sw_to_lega_16_19 ~ dummy_diesel + dummy_euro_4 + diesel_euro4,
  data = data,
  subset = c(
    target != 3 &
      target != 4 & no_answer_euro == 0 & no_answer_municipal == 0
  )
)

# 2. with controls
Legaswitch_munic_lm_cont <- estimatr::lm_robust(
  sw_to_lega_16_19 ~ dummy_diesel + dummy_euro_4 + diesel_euro4 +
    age + female + EDU + INC,
  data = data,
  subset = c(
    target != 3 & target != 4 &
      no_answer_euro ==
      0 & no_answer_municipal == 0
  )
)

# 3. Assigning Unknown-car models based on self-report of ban effect
Legaswitch_munic_lm_cont_Unknowncar <- estimatr::lm_robust(
  sw_to_lega_16_19 ~ dummy_diesel_ass + dummy_euro_4_ass + diesel_euro4_ass +
    age + female + EDU + INC +
    dummy_car_unknown,
  data = data,
  subset = c(target != 3 &
...
Legaswitch_munic_all <- bind_rows(
  M.Legaswitch_munic_lm_ms,
  M.Legaswitch_munic_lm_cont,
  M.Legaswitch_munic_lm_cont_Unknowncar,
  .id = "Model"
)
Legaswitch_munic_all <- subset(
  Legaswitch_munic_all,
  term == "dummy_diesel" | term == "dummy_euro_4" |
    term == "diesel_euro4" |
    term == "dummy_diesel_ass" |
    term == "dummy_euro_4_ass" | term == "diesel_euro4_ass"
...
Legaswitch_munic_all$Model <- factor(
  Legaswitch_munic_all$Model,
  levels = c("1", "2", "3"),
  labels = c("No Controls", "With Controls", "Incl. Noncategorized Cars")
)
...
# 1. no control
pl16_18v2_lm_ms <- estimatr::lm_robust(
  sw_to_lega_16_18 ~ dummy_diesel + dummy_euro_4 + diesel_euro4,
  data = data,
  subset = c(
    target != 3 &
      target != 4 &
      no_answer_2018 == 0 &
      no_answer_municipal == 0 & vote_lega_municipal == 0
  )
)
# 2. with controls
pl16_18v2_lm_cont <- estimatr::lm_robust(
  sw_to_lega_16_18 ~ dummy_diesel + dummy_euro_4 + diesel_euro4 +
    age + female + EDU + INC,
  data = data,
  subset = c(
...
pl16_18v2_lm_cont_Unkncar <- estimatr::lm_robust(
  sw_to_lega_16_18 ~ dummy_diesel_ass + dummy_euro_4_ass + diesel_euro4_ass +
...
M.pl16_18v_all_main$Model <- factor(
  M.pl16_18v_all_main$Model,
  levels = c("1", "2", "3"),
  labels = c("No Controls", "With Controls", "Incl. Noncategorized Cars")
)
...
M.pl16_18v_all_main$term3 <- factor(M.pl16_18v_all_main$term2,
                                    levels = c("Euro 4", "Diesel", "Diesel Euro 4"))

## coefplot
M.pl16_18v_all_mainsub_Sub <- subset(M.pl16_18v_all_main, term2 == "Diesel Euro 4")
M.pl16_18v_all_mainsub_Sub$Model2 <- factor(
  M.pl16_18v_all_mainsub_Sub$Model,
  levels = c("Incl. Noncategorized Cars", "With Controls", "No Controls")
)

# Panel (b) From municipal 2016 to regional 2018  ----------
# 1. no control
pl16_regv2_lm_ms <- estimatr::lm_robust(
  sw_to_lega_16_reg ~ dummy_diesel + dummy_euro_4 + diesel_euro4,
  data = data,
  subset = c(
    target != 3 &
      target != 4 &
...
# 2. with controls
pl16_regv2_lm_cont <- estimatr::lm_robust(
  sw_to_lega_16_reg ~ dummy_diesel + dummy_euro_4 + diesel_euro4 +
    age + female + EDU + INC,
  data = data,
  subset = c(
...
pl16_regv2_lm_cont_Unkncar <- estimatr::lm_robust(
  sw_to_lega_16_reg ~ dummy_diesel_ass + dummy_euro_4_ass + diesel_euro4_ass +
    age + female + EDU + INC + dummy_car_unknown,
  data = data,
  subset = c(
    target != 3 &
...
M.pl16_regv_all_main <- bind_rows(M.pl16_regv2_lm_ms,
                                  ...
                                  .id = "Model")
M.pl16_regv_all_main <- subset(
  M.pl16_regv_all_main,
  term == "dummy_diesel" | term == "dummy_euro_4" |
    term == "diesel_euro4" |
    term == "dummy_diesel_ass" |
    term == "dummy_euro_4_ass" | term == "diesel_euro4_ass"
...
M.pl16_regv_all_main$Model <- factor(
  M.pl16_regv_all_main$Model,
  levels = c("1", "2", "3"),
  labels = c("No Controls", "With Controls", "Incl. Noncategorized Cars")
)
M.pl16_regv_all_main$term2 <- relevel(M.pl16_regv_all_main$term2, ref =
                                        "Diesel Euro 4")
M.pl16_regv_all_main$term2 <- relevel(M.pl16_regv_all_main$term2, ref =
                                        "Euro 4")
M.pl16_regv_all_main$term3 <- factor(M.pl16_regv_all_main$term2,
                                     levels = c("Euro 4", "Diesel", "Diesel Euro 4"))
M.pl16_regv_all_mainsub_Sub <- subset(M.pl16_regv_all_main, term2 == "Diesel Euro 4")
M.pl16_regv_all_mainsub_Sub$Model2 <- factor(
  M.pl16_regv_all_mainsub_Sub$Model,
  levels = c("Incl. Noncategorized Cars", "With Controls", "No Controls")
)
...
switch_16_19_placebo$election18 <- factor(
  switch_16_19_placebo$election18,
  levels = c("1", "2"),
  labels = c(
    "(a) From Municipal 2016 to Legislative 2018",
...
coef_switch_16_19_placebo <- ggplot(switch_16_19_placebo, aes(Model2, estimate)) +
  ...
  labs(x = "", y = " ", title = " ") + facet_wrap(facets =  ~ election18, nrow =
                                                    1)
